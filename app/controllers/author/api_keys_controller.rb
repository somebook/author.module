module Author
class ApiKeysController < SpaceController
  ...
  def create
    ...
    if @api_key.save
      redirect_to api_keys_path, notice: t("author.api_key.notice.create_success")
    else
      render action: "new"
    end
  end
  ...
  def update
    ...
    if @api_key.update_attributes(params[:api_key])
      redirect_to api_keys_path, notice: t("author.api_key.notice.update_success")
    else
      render action: "edit"
    end
  end
