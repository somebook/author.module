class Author::AlbumsController < Author::PicasaController
  ...
  def create
    ...
    if @album.save_to_db_and_picasa(@picasa, current_user, @current_shard)
      redirect_to author_albums_path, notice: 'Album was successfully created.'
    else
      redirect_to new_author_album_path, error: 'Album creation fails.'
    end
  end
  ...
  def destroy
    ...
    if @album.destroy_from_db_and_picasa(@picasa)
      redirect_to author_albums_path, notice: 'Album was successfully deleted.'
    else
      redirect_to author_albums_path, error: 'There were errors while deleting album.'
    end
  end
  ...
  def update
    ...
    if @album.update_in_db_and_picasa(@picasa, params[:album])
      redirect_to author_albums_path, notice: 'Album was successfully updated.'
    else
      redirect_to edit_author_album_path(@album), error: 'There were errors while deleting album.'
    end
  end
  ...
  def sync
    ...
    redirect_to author_albums_path, notice: "Albums was successfully synchronized."
  end
  ...
  def authorize
    if Picasa.token_in_request?(request)
      begin
        ...
        redirect_to author_albums_path, notice: 'Picasa authorization complete.'
      rescue
        redirect_to author_albums_path, error: 'Picasa authorization fail.'
      end
    end
  end
  ...
  def hidden
    ...
    redirect_to author_albums_path, notice: "Album was successfully #{'un' unless @album.hidden}hidden."
  end

  private

  def check_picasa_auth
    redirect_to author_albums_url if @picasa.nil?
  end
